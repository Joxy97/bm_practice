...
Updated to use BoltzmannMachine abstraction instead of direct GRBM.
"""
...
class BoltzmannMachineTrainer:
    ...
    Works with BoltzmannMachine abstraction layer, using samplers from
    sampler_dict by name.
    """
    ...
    def __init__(
        ...
        Args:
            model: BoltzmannMachine instance (abstraction over GRBM)
            config: Training configuration dictionary
            device: torch device (CPU or CUDA)
            sampler_name: Name of sampler to use from model's sampler_dict
        """
        self.device = device
        ...
        self.sampler = model.sampler_dict.get(sampler_name)
        if self.sampler is None:
            raise ValueError(
                f"Sampler '{sampler_name}' not found in model's sampler_dict. "
                f"Available: {list(model.sampler_dict.keys())}"
            )
